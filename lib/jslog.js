/*global define, global, amd, window*/
/*jslint white: true*/
/*jshint multistr: true */
(function (f, define) {
    'use strict';
    define([], f);
})(function () {
    'use strict';
    var JsLogExport = null,
        LOGGER_HOST = 'jslog.me',
        LOGGER_PATH = '/log',
        LOGGER_URL = '//' + LOGGER_HOST + LOGGER_PATH,
        SEND_POSTPONE_INTERVAL = 10000,
        SERVER_REQUEST_TIMEOUT = 10000,
        MAX_PACKET_SIZE = 1024 * 1024,
        MAX_PENDING_PACKET_SIZE = 5240,
        DEFAULT_MAX_POSTPONE_SIZE = 1024 * 1024,
        JSLOG_ME_LOCALSTORAGE = 'JSLOG_ME_POSTPONED_ITEMS',
        DEFAULT_PROTOCOL = 'http:',
        hostId,
        availableConsoleHookOptions = ['log', 'warn', 'info', 'error'];

        function bind(context, fn) {
            return function () {
                return fn.apply(context, arguments);
            };
        }

        function getEpochTime() {
            if (Date.now) {
                return Date.now();
            } else {
                return (new Date()).getTime();
            }
        }

        var JsLogOptions = (function () {
                function JsLogOptions() {
                    // Main settings
                    this.enabled = true;
                    this.key = '';
                    this.version = '';
                    this.sessionId = this.guid();
                    // Old-style settings, deprecated
                    this.hookConsole = true;
                    this.collectSystemInfo = true;
                    this.trackLaunches = true;
                    // New-style settings
                    this.logUncaughtExceptions = true;
                    this.console = {
                        enabled: true,
                        display: true,
                        watch: ['log', 'info', 'warn', 'error']
                    };
                    this.network = {
                        enabled: true,
                        codes: [],
                        codesExclude: [200, 304, 404],
                        logRequest: true,
                        logResponse: true
                    };
                    this.userInteraction = {
                        enabled: false,
                        events: ['click', 'dblclick', 'change', 'tap', 'drag', 'drop'],
                        navigation: true
                    };
                }

                JsLogOptions.prototype.guid = (function () {
                    function s4() {
                        return Math.floor((1 + Math.random()) * 0x10000)
                            .toString(16)
                            .substring(1);
                    }

                    return function () {
                        return s4() + s4() + '-' + s4() + '-' + s4() + '-' +
                            s4() + '-' + s4() + s4() + s4();
                    };
                })();

                /**
                 * Assign options from specified source
                 * @param {JsLogOptions} options
                 */
                JsLogOptions.prototype.assign = function (options) {
                    function assignObject(target, options) {
                        if (!options) return;
                        //
                        for (var idx in options) {
                            if (options.hasOwnProperty(idx) && target.hasOwnProperty(idx)) {
                                if (target[idx] instanceof Array) {
                                    target[idx] = Array.prototype.slice.call(options[idx]);
                                } else if (typeof target[idx] === 'object') {
                                    assignObject(target[idx], options[idx]);
                                } else {
                                    target[idx] = options[idx];
                                }
                            }
                        }
                    }

                    //
                    assignObject(this, options);
                    //
                    if (options && options.console && (typeof options.console.enabled !== 'undefined')) {
                        this.hookConsole = options.console.enabled;
                    }
                };

                return JsLogOptions;
            })(),

            SenderService = (function () {
                function SenderService() {
                    this.inOperation = false;
                    this.xmlHttpRequest = createRequest();
                }

                /**
                 * Get HTTP request object
                 *
                 * @returns XMLHttpRequest
                 */
                function createRequest() {
                    function tryActiveX(activeX) {
                        try {
                            return new ActiveXObject(activeX);
                        }
                        catch (e) {
                        }
                    }

                    var XMLHttpRequest = window.XMLHttpRequest;
                    if (typeof XMLHttpRequest === 'undefined') {
                        return tryActiveX("Msxml2.XMLHTTP.6.0") ||
                            tryActiveX("Msxml2.XMLHTTP.3.0") ||
                            tryActiveX("Msxml2.XMLHTTP") ||
                            tryActiveX("Microsoft.XMLHTTP");
                    } else {
                        return new XMLHttpRequest();
                    }
                }

                SenderService.prototype.sendXmlHttpRequest = function (protocol, postData, successHandler, failHandler) {
                    var that = this,
                        xmlHttpRequest = this.xmlHttpRequest;
                    //
                    this.inOperation = true;
                    xmlHttpRequest.open("POST", protocol + LOGGER_URL, true);
                    xmlHttpRequest.timeout = SERVER_REQUEST_TIMEOUT;
                    xmlHttpRequest.setRequestHeader("Content-Type", 'application/json; charset=UTF-8');
                    xmlHttpRequest.onreadystatechange = function () {
                        if (xmlHttpRequest.readyState === 4) {
                            that.inOperation = false;
                            if (xmlHttpRequest.status === 200 || xmlHttpRequest.status === 400) {
                                // Request sent successfully
                                successHandler(postData);
                            } else {
                                // Fail handler
                                failHandler(postData);
                            }
                        }
                    };
                    try {
                        xmlHttpRequest.send(postData);
                    } catch (e) {
                        failHandler(postData);
                    }
                };

                /**
                 *
                 * @param {string} protocol
                 * @param {string} postData
                 * @param {boolean} forceSend
                 * @param {Function} successHandler
                 * @param {Function} failHandler
                 */
                SenderService.prototype.sendPacket = function (protocol, postData, forceSend, successHandler, failHandler) {
                    if (this.inOperation) {
                        if (!forceSend) {
                            failHandler(postData);
                            return;
                        }
                    }
                    if (this.xmlHttpRequest) {
                        this.sendXmlHttpRequest(protocol, postData, successHandler, failHandler);
                    }
                };
                return SenderService;
            })(),

            PendingQueue = (function () {
                function PendingQueue() {
                    this.clear();
                    this.loadPostponedData();
                }

                /**
                 * Clear pending queue
                 */
                PendingQueue.prototype.clear = function () {
                    this.timePrefix = getEpochTime() + '';
                    this.counter = 0;
                    this.postponed = {};
                    this.postponedLength = 0;
                    this.postponedSize = 0;
                    this.lastItem = undefined;
                };

                /**
                 * Clear all postponed logs from the local storage
                 */
                PendingQueue.prototype.clearWithPostponedData = function () {
                    try {
                        if (typeof window === 'object' && window && window.localStorage) {
                            for (var idx in window.localStorage) {
                                if (window.localStorage.hasOwnProperty(idx) && (idx.indexOf(JSLOG_ME_LOCALSTORAGE) === 0)) {
                                    delete window.localStorage[idx];
                                }
                            }
                        }
                        this.clear();
                    } catch (e) {

                    }
                };

                /**
                 * Initial load of pending queue stored in local storage
                 * @return {*}
                 */
                PendingQueue.prototype.loadPostponedData = function () {
                    if (typeof window === 'object' && window && window.localStorage) {
                        try {
                            this.postponed = {};
                            this.lastItem = undefined;
                            this.postponedSize = 0;
                            var prefixLength = JSLOG_ME_LOCALSTORAGE.length,
                                count = 0;
                            for (var idx in window.localStorage) {
                                if (window.localStorage.hasOwnProperty(idx) && (idx.indexOf(JSLOG_ME_LOCALSTORAGE) === 0)) {
                                    var key = idx.substr(prefixLength);
                                    this.lastItem = key;
                                    this.postponed[key] = window.localStorage[idx];
                                    this.postponedSize = this.postponedSize + this.postponed[key].length;
                                    count++;
                                }
                            }
                            this.postponedLength = count;
                            if (count) {
                                return this.postponed;
                            }
                        } catch (e) {
                        }
                    }
                };

                /**
                 * Store postponed item
                 * @param {string} lastItem
                 * @param {string} packetData
                 */
                function storePostponedItem(lastItem, packetData) {
                    if (typeof window === 'object' && window && window.localStorage) {
                        try {
                            window.localStorage[JSLOG_ME_LOCALSTORAGE + lastItem] = packetData;
                        } catch (e) {
                        }
                    }
                }

                /**
                 * Add packet
                 * @param {string} packedData
                 */
                PendingQueue.prototype.add = function (packedData) {
                    function appendNewItem() {
                        that.counter++;
                        that.lastItem = that.timePrefix + that.counter;
                        that.postponed[that.lastItem] = packedData;
                        that.postponedSize = that.postponedSize + packedData.length;
                        storePostponedItem(that.lastItem, packedData);
                    }

                    var that = this;
                    if (!packedData || typeof packedData !== 'string' || packedData.charAt(0) !== '[') {
                        throw new Error("Invalid log message - not an array");
                    }
                    if (that.postponedSize > DEFAULT_MAX_POSTPONE_SIZE) {
                        return;
                    }
                    if (this.lastItem) {
                        var lastData = this.postponed[that.lastItem];
                        if (lastData.length > MAX_PENDING_PACKET_SIZE) {
                            appendNewItem();
                            this.postponedLength++;
                        } else {
                            var newData = mergePackedArrays(lastData, packedData);
                            that.postponed[that.lastItem] = newData;
                            storePostponedItem(that.lastItem, newData);
                            that.postponedSize = that.postponedSize + newData.length - lastData.length;
                        }
                    } else {
                        appendNewItem();
                        this.postponedLength++;
                    }
                };

                var PACKET_REGEXP = /^\s*\[\s*(.+)\s*]\s*$/;

                /**
                 *
                 * @param {string} packedArray1
                 * @param {string} packedArray2
                 */
                function mergePackedArrays(packedArray1, packedArray2) {
                    var internals1 = packedArray1.match(PACKET_REGEXP),
                        internals2 = packedArray2.match(PACKET_REGEXP);
                    if (internals1 && internals2 && (internals1.length > 1) && (internals2.length > 1)) {
                        return '[' + internals1[1] + ', ' + internals2[1] + ']';
                    } else if (internals1) {
                        return packedArray1;
                    } else if (internals2) {
                        return packedArray2;
                    } else {
                        return '[]';
                    }
                }

                /**
                 * Returns number of postponed items
                 * @return {Number}
                 */
                PendingQueue.prototype.haveItems = function () {
                    return this.postponedLength;
                };

                /**
                 * Fetch a single item from pending queue
                 * It does not remove item from the queue
                 *
                 * @return {*}
                 */
                PendingQueue.prototype.fetchItem = function () {
                    if (!this.postponedLength) {
                        return undefined;
                    }
                    for (var idx in this.postponed) {
                        if (this.postponed.hasOwnProperty(idx)) {
                            return {
                                key: idx,
                                value: this.postponed[idx]
                            };
                        }
                    }
                    return undefined;
                };

                /**
                 * Delete item from pending queue
                 * @param {string} itemName
                 */
                PendingQueue.prototype.deleteItem = function (itemName) {
                    if (typeof this.postponed[itemName] === 'undefined') {
                        return;
                    }
                    this.postponedLength--;
                    this.postponedSize = this.postponedSize - this.postponed[itemName].length;
                    delete this.postponed[itemName];
                    if (typeof window === 'object' && window && window.localStorage) {
                        try {
                            delete window.localStorage[JSLOG_ME_LOCALSTORAGE + itemName];
                        } catch (e) {
                        }
                    }
                    if (this.lastItem === itemName) {
                        this.lastItem = undefined;
                    }
                };
                return PendingQueue;
            })(),

            PendingSenderService = (function () {
                function PendingSenderService(protocol, /**SenderService*/sender, /**PendingQueue*/queue) {
                    this.protocol = protocol;
                    /**
                     * @type {SenderService}
                     */
                    this.sender = sender;
                    /**
                     * @type {PendingQueue}
                     */
                    this.queue = queue;
                }

                PendingSenderService.SEND_POSTPONE_INTERVAL = SEND_POSTPONE_INTERVAL;

                PendingSenderService.prototype.sendPendingItem = function () {
                    if (this.sender.inOperation || !this.queue.haveItems()) {
                        return;
                    }
                    var item = this.queue.fetchItem(),
                        that = this;
                    if (item) {
                        var key = item.key,
                            value = item.value;
                        this.sender.sendPacket(this.protocol, value, false, function () {
                            // success handler:
                            // delete from queue, send next
                            that.queue.deleteItem(key);
                            that.sendPendingItem();
                        }, function () {
                            // fail handler:
                            // nothing to do, retry later
                        });
                    }
                };

                PendingSenderService.prototype.start = function () {
                    if (this.handle) {
                        return;
                    }
                    var that = this;
                    this.handle = setInterval(function () {
                        that.sendPendingItem();
                    }, SEND_POSTPONE_INTERVAL);
                };

                PendingSenderService.prototype.stop = function () {
                    if (this.handle) {
                        clearInterval(this.handle);
                        this.handle = undefined;
                    }
                };
                return PendingSenderService;
            })(),

            JsLog = (function () {
                function JsLog(options) {
                    this.options = new JsLogOptions();
                    this.options.assign(options);
                    var protocol;
                    if (typeof window !== 'undefined') {
                        protocol = window.location.protocol;
                        if (!((protocol.toLowerCase() === 'http:') || (protocol.toLowerCase() === 'https:'))) {
                            protocol = DEFAULT_PROTOCOL;
                        }
                    } else {
                        protocol = DEFAULT_PROTOCOL;
                    }
                    this.protocol = protocol;
                    this.systemInfoSent = false;
                    /**
                     * @type {string[]}
                     */
                    this._batchQueue = [];
                    this.senderService = new SenderService();
                    this.pendingSenderService = new PendingSenderService(protocol, this.senderService, pendingQueue);
                    this.init();
                    this.pendingSenderService.start();
                }

                /**
                 * Raw sender
                 * @param {string} protocol
                 * @param {JsLog} that
                 * @param {string|undefined} postData
                 * @param {boolean} [forceSend=false]
                 */
                function rawSendToServer(protocol, that, postData, forceSend) {
                    forceSend = !!forceSend;
                    if (batchQueueSize(that) > MAX_PACKET_SIZE) {
                        var oldData = generatePacketData(that);
                        that.postpone(oldData);
                    }
                    if (postData) {
                        addToBatch(that, postData);
                    }
                    if ((!that.senderService.inOperation || forceSend) && that._batchQueue.length) {
                        var packetData = generatePacketData(that);
                        that.senderService.sendPacket(protocol, packetData, forceSend, function () {
                            // Success handler:
                            // send next portion if available
                            rawSendToServer(protocol, that, undefined, false);
                        }, function () {
                            // Fail handler:
                            // postpone sending
                            that.postpone(packetData);
                        });
                    }
                }

                /**
                 * Add message to a batch
                 *
                 * @param {JsLog} that
                 * @param {string} postData
                 */
                function addToBatch(that, postData) {
                    that._batchQueue.push(postData);
                }

                /**
                 * Generate a packet from queue
                 * and clears queue
                 *
                 * @param {JsLog} that
                 * @returns string
                 */
                function generatePacketData(that) {
                    var result = '[' + that._batchQueue.join(', ') + ']';
                    that._batchQueue = [];
                    return result;
                }

                /**
                 * Get apx queue size in chars
                 * @param {JsLog} that
                 * @return {number}
                 */
                function batchQueueSize(that) {
                    var size = 0;
                    for (var i = 0; i < that._batchQueue.length; i++) {
                        size = size + that._batchQueue[i].length;
                    }
                    return size;
                }

                JsLog.prototype.renewSession = function () {
                    this.options.sessionId = this.options.guid();
                    this.systemInfoSent = false;
                };

                JsLog.prototype.sendToServer = function (key, eventType, data) {
                    if (!this.options.enabled) {
                        return;
                    }
                    if (data instanceof Error) {
                        data = exceptionToObject(data);
                    }
                    var rawData = {
                            key: key,
                            sessionId: this.options.sessionId,
                            hostId: hostId,
                            time: getEpochTime(),
                            type: eventType,
                            data: data
                        },
                        packetData = JSON.stringify(rawData);

                    rawSendToServer(this.protocol, this, packetData);
                };

                function initialSystemInfo(/**JsLog*/log) {
                    if (!log.systemInfoSent) {
                        log.systemInfo();
                    }
                }

                JsLog.prototype.postpone = function (packetData) {
                    pendingQueue.add(packetData);
                };

                JsLog.prototype.expand = function (data) {
                    var visitedObjects = [];

                    function expandOneLevel(data) {
                        if (typeof data !== 'object' || data === null) {
                            return data;
                        }
                        var result = (data instanceof Array) ? [] : {};
                        visitedObjects.push(data);
                        var properties = getProperties(data);
                        properties.sort();
                        for (var idx in properties) {
                            if (properties.hasOwnProperty(idx)) {
                                var propertyName = properties[idx];
                                var propertyValue = data[propertyName];
                                if (typeof propertyValue === 'undefined' || typeof propertyValue === 'function') {
                                    continue;
                                }
                                if (visitedObjects.indexOf(propertyValue) >= 0) {
                                    propertyValue = '<circular ref>';
                                }
                                result[propertyName] = expandOneLevel(propertyValue);
                            }
                        }
                        visitedObjects.pop();
                        return result;
                    }

                    return expandOneLevel(data);
                };

                function getProperties(object) {
                    var result = [];
                    (function extractOneLevelNames(object) {
                        if (typeof object !== 'object' || !object) {
                            return;
                        }
                        result = result.concat(Object.getOwnPropertyNames(object));
                        extractOneLevelNames(Object.getPrototypeOf(object));
                    })(object);
                    return result;
                }

                /**
                 * Send and log to console
                 *
                 * @param {JsLog} jsLog
                 * @param {string} type
                 * @param {*} args
                 */
                function sendUnifiedLogEntry(jsLog, type, args) {
                    try {
                        initialSystemInfo(jsLog);
                        var sendArgs = (args.length > 1) ? args : args[0],
                            expandedParameters = expandParameters(sendArgs);
                        jsLog.sendToServer(jsLog.options.key, type, expandedParameters);
                        if (jsLog.options.console.display) {
                            if (typeof oldConsole[type] === 'function') {
                                oldConsole[type].apply(window.console, args);
                            } else if (window && window.console && typeof window.console[type] === 'function') {
                                window.console[type].apply(window.console, args);
                            }

                        }
                    } catch (e) {
                    }
                }

                function expandParameters(sendArgs) {
                    if (typeof sendArgs === 'object' && sendArgs && sendArgs.length) {
                        // multiple args
                        var result = [];
                        for (var idx in sendArgs) {
                            if (sendArgs.hasOwnProperty(idx)) {
                                result.push(JsLog.prototype.expand(sendArgs[idx]));
                            }
                        }
                        return result;
                    } else {
                        // single arg
                        return JsLog.prototype.expand(sendArgs);
                    }
                }

                JsLog.prototype.log = function () {
                    return sendUnifiedLogEntry(this, 'log', Array.prototype.slice.call(arguments));
                };

                JsLog.prototype.info = function () {
                    return sendUnifiedLogEntry(this, 'info', Array.prototype.slice.call(arguments));
                };

                JsLog.prototype.warn = function () {
                    return sendUnifiedLogEntry(this, 'warn', Array.prototype.slice.call(arguments));
                };

                JsLog.prototype.error = function () {
                    return sendUnifiedLogEntry(this, 'error', Array.prototype.slice.call(arguments));
                };

                JsLog.prototype.exception = function (data) {
                    return sendUnifiedLogEntry(this, 'exception', Array.prototype.slice.call(arguments));
                };

                JsLog.prototype.network = function () {
                    return sendUnifiedLogEntry(this, 'network', Array.prototype.slice.call(arguments));
                };

                JsLog.prototype.systemInfo = function () {
                    try {
                        var systemInfoData = {};
                        if (typeof navigator === 'object') {
                            systemInfoData.userAgent = navigator.userAgent;
                            systemInfoData.platform = navigator.platform;
                            systemInfoData.version = this.options.version;
                        } else {
                            systemInfoData.userAgent = '';
                            systemInfoData.platform = '';
                            systemInfoData.version = this.options.version;
                        }
                        this.systemInfoSent = true;
                        this.sendToServer(this.options.key, 'systemInfo', systemInfoData);
                    } catch (e) {
                    }
                };

                function exceptionToObject(exception) {
                    var result = {};
                    if (!exception) {
                        return result;
                    }
                    if (typeof exception.name !== 'undefined') {
                        result.name = exception.name;
                    }
                    if (typeof exception.message !== 'undefined') {
                        result.message = exception.message;
                    }
                    if (typeof exception.stack !== 'undefined') {
                        result.stack = exception.stack;
                    }
                    if (typeof exception.fileName !== 'undefined') {
                        result.fileName = exception.fileName;
                    }
                    if (typeof exception.lineNumber !== 'undefined') {
                        result.lineNumber = exception.lineNumber;
                    }
                    if (typeof exception.columnNumber !== 'undefined') {
                        result.columnNumber = exception.columnNumber;
                    }
                    if (typeof exception.description !== 'undefined') {
                        result.description = exception.description;
                    }
                    return result;
                }

                JsLog.prototype.exceptionToObject = exceptionToObject;

                JsLog.prototype.init = function () {
                    if (!this.options.enabled) {
                        return;
                    }
                    this.pendingSenderService.sendPendingItem();
                    trackHost();
                    if (this.options.hookConsole) {
                        this.hookConsole();
                    }
                    if (this.options.network.enabled) {
                        this.hookNetwork();
                    }
                    if (this.options.logUncaughtExceptions) {
                        this.hookUncaughtExceptions();
                    }
                    if (this.options.trackLaunches) {
                        this.systemInfo();
                    }
                };

                JsLog.prototype.finalize = function () {
                    this.unhookNetwork();
                    this.unhookConsole();
                };

                var oldConsole = {};

                function trackHost() {
                    if (hostId) {
                        return;
                    }
                    // put tracking option in local storage
                    if (typeof window === 'object' && 'localStorage' in window && window.localStorage !== null) {
                        try {
                            hostId = window.localStorage.getItem('JsLogToken');
                        } catch (e) {
                            hostId = null;
                        }
                        if (!hostId) {
                            hostId = JsLogOptions.prototype.guid();
                            try {
                                window.localStorage.setItem('JsLogToken', hostId);
                            } catch (e) {
                            }
                        }
                    } else {
                        hostId = JsLogOptions.prototype.guid();
                    }
                }

                JsLog.prototype.hookConsole = function () {
                    if (typeof window.console !== 'undefined') {
                        for (var idx in this.options.console.watch) {
                            if (this.options.console.watch.hasOwnProperty(idx)) {
                                var item = this.options.console.watch[idx];
                                if (availableConsoleHookOptions.indexOf(item) >= 0) {
                                    oldConsole[item] = window.console[item];
                                    window.console[item] = bind(this, this[item]);
                                }
                            }
                        }
                    }
                };

                JsLog.prototype.unhookConsole = function () {
                    if (typeof window.console !== 'undefined') {
                        for (var idx in oldConsole) {
                            if (oldConsole.hasOwnProperty(idx)) {
                                window.console[idx] = oldConsole[idx];
                            }
                        }
                    }
                };

                var oldNetworkSend = null,
                    oldNetworkOpen = null;

                /**
                 *
                 * @param {XMLHttpRequest} xhr
                 * @param {JsLog} logger
                 */
                function waitForNetworkCompletion(xhr, logger) {
                    var oldOnError = xhr.onerror,
                        oldTimeOut = xhr.ontimeout,
                        oldOnLoad = xhr.onload;

                    function revertHandlers() {
                        xhr.onerror = oldOnError;
                        xhr.ontimeout = oldTimeOut;
                        xhr.onload = oldOnLoad;
                    }

                    function logNetwork(success) {
                        try {
                            var info = xhr._additionalInfo || {},
                                networkOptions = logger.options.network;
                            info.status = xhr.status || 0;
                            info.statusText = xhr.statusText || '';
                            if (logger.options.network.logResponse) {
                                info.response = xhr.responseText;
                                try {
                                    info.responseHeaders = xhr.getAllResponseHeaders();
                                } catch (e) {
                                    info.responseHeaders = {};
                                }
                            }
                            var codeAllowed = (networkOptions.codes.indexOf(xhr.status) >= 0) ||
                                (!networkOptions.codes.length && (networkOptions.codesExclude.indexOf(xhr.status) < 0)),
                                // XHR could return 0 status on a success call, when requesting local file protocol.
                                // We assume it is equal with 200 response code
                                successWithZeroStatus = success && !info.status && (networkOptions.codes.indexOf(200) >= 0);
                            if (networkOptions.enabled && (codeAllowed || successWithZeroStatus)) {
                                logger.network(info);
                            }
                        } catch (e) {
                        }
                    }

                    xhr.onerror = function () {
                        logNetwork(false);
                        revertHandlers();
                        if (typeof oldOnError === 'function') {
                            oldOnError.apply(this, arguments);
                        }
                    };

                    xhr.ontimeout = function () {
                        logNetwork(false);
                        revertHandlers();
                        if (typeof oldTimeOut === 'function') {
                            oldTimeOut.apply(this, arguments);
                        }
                    };

                    xhr.onload = function () {
                        logNetwork(true);
                        revertHandlers();
                        if (typeof oldOnLoad === 'function') {
                            oldOnLoad.apply(this, arguments);
                        }
                    };

                }

                JsLog.prototype.hookNetwork = function () {
                    var that = this;
                    if (oldNetworkSend) return;
                    if (window.XMLHttpRequest) {
                        var fn = window.XMLHttpRequest.prototype;
                        oldNetworkSend = fn.send;
                        oldNetworkOpen = fn.open;
                        fn.open = function (method, url) {
                            if (url.indexOf(LOGGER_URL) < 0) {
                                this._additionalInfo = {
                                    method: method,
                                    url: url
                                };
                            }
                            return oldNetworkOpen.apply(this, arguments);
                        };
                        fn.send = function (data) {
                            if (!this._additionalInfo) {
                                // dont report on itself
                                return oldNetworkSend.apply(this, arguments);
                            }
                            if (that.options.network.logRequest) {
                                this._additionalInfo.requestData = data;
                            }
                            waitForNetworkCompletion(this, that);
                            return oldNetworkSend.apply(this, arguments);
                        };
                    }
                };

                JsLog.prototype.unhookNetwork = function () {
                    var fn = window.XMLHttpRequest.prototype;
                    if (oldNetworkSend) {
                        fn.send = oldNetworkSend;
                        fn.open = oldNetworkOpen;
                        oldNetworkSend = null;
                        oldNetworkOpen = null;
                    }
                };

                JsLog.prototype.hookUncaughtExceptions = function () {
                    // Browser
                    if (typeof window !== 'undefined') {
                        window.onerror = bind(this, this.windowErrorHandler);
                    }
                };

                JsLog.prototype.windowErrorHandler = function (msg, url, line, column, errorObj) {
                    if (!this.options.enabled) {
                        return;
                    }
                    try {
                        if (errorObj) {
                            this.sendToServer(this.options.key, 'uncaughtException',
                                {
                                    msg: msg,
                                    url: url,
                                    line: line,
                                    column: column,
                                    errorObj: exceptionToObject(errorObj)
                                });
                        } else {
                            this.sendToServer(this.options.key, 'uncaughtException',
                                exceptionToObject(msg));
                        }
                    } catch (e) {

                    }
                };

                return JsLog;
            })();


        var pendingQueue = new PendingQueue();
        //
        JsLog.JsLogOptions = JsLogOptions;
        JsLog.PendingQueue = PendingQueue;
        JsLog.SenderService = SenderService;
        JsLog.PendingSenderService = PendingSenderService;

        // singleton
    if (typeof window !== 'undefined') {
        window.JsLog = window.JsLog || JsLog;
    }
    JsLogExport = JsLog;
    return JsLogExport;
}, typeof define == 'function' && define.amd ? define : function (_, f) {
    f();
});
